...
function addAdultAPI() {
    ...
    if (!HIDE_BUILTIN_ADULT_APIS && (localStorage.getItem('yellowFilterEnabled') === 'false')) {
        ...
        adultTitle.innerHTML = `裸露資源採集站 <span class="adult-warning">
...
function checkAdultAPIsSelected() {
    ...
    if (hasAdultSelected) {
        ...
        if (filterDescription) {
            filterDescription.innerHTML = '<strong class="text-pink-300">選擇裸露資源時無法啟用此過濾</strong>';
        }
        ...
        // 恢复原来的描述文字
        if (filterDescription) {
            filterDescription.innerHTML = '濾除過於裸露等内容';
        }
...
function resetSearchArea() {
    ...
    try {
        window.history.pushState(
            {},
            `NovaTV - 免費線上影視搜索與觀看平台`,
            `/`
        );
        // 更新页面标题
        document.title = `NovaTV - 免費線上影視搜索與觀看平台`;
    } catch (e) {
...
async function search() {
    ...
    try {
        ...
        if (!allResults || allResults.length === 0) {
            resultsDiv.innerHTML = `
                <div class="col-span-full text-center py-16">
                    ...
                    <h3 class="mt-2 text-lg font-medium text-gray-400">没有找到相符的结果</h3>
        ...
        try {
            ...
            // 更新页面标题
            document.title = `搜索: ${query} - NovaTV`;
        } catch (e) {
            console.error('更新瀏覽器歷史失敗:', e);
            // 如果更新URL失败，继续执行搜索
        }
        ...
        const safeResults = allResults.map(item => {
            ...
            let displayCover = item.vod_pic || '';
            if (!displayCover || displayCover === 'null' || displayCover === 'undefined' || !displayCover.startsWith('http')) {
                displayCover = '/image/movie.png';
            } else if (displayCover.startsWith('http:')) {
                // 處理混合內容 (HTTP 圖片在 HTTPS 站點)
                displayCover = PROXY_URL + encodeURIComponent(displayCover);
            }
                ...
                <div class="card-hover bg-[#111] rounded-lg overflow-hidden cursor-pointer transition-all hover:scale-[1.02] h-full shadow-sm hover:shadow-md" 
                    ...
                    <div class="flex h-full">
                        <div class="relative flex-shrink-0 search-card-img-container">
                            <img src="${displayCover}" alt="${safeName}" 
                                 class="h-full w-full ${displayCover.includes('movie.png') ? 'object-contain' : 'object-cover'} transition-transform hover:scale-110" 
                                 onerror="handlePosterError(this, '${safeName}')" 
                    ...
                    `<span class="text-xs py-0.5 px-1.5 rounded bg-opacity-20 bg-purple-500 text-purple-300">
                                ...
                                <p class="text-gray-400 line-clamp-2 overflow-hidden mb-2">
                                    ${(item.vod_remarks || '暫無介绍').toString().replace(/</g, '&lt;')}
                                </p>
                            </div>
        ...
        if (error.name === 'AbortError') {
            showToast('搜索請求超時，請檢查網路連線', 'error');
        } else {
            showToast('搜索請求失敗，請稍後再試', 'error');
        }
    } finally {
        hideLoading();
    }
...
function handlePlayerError() {
    hideLoading();
    showToast('影片播放加載失敗，請嘗試其他影片', 'error');
}
...
async function importConfigFromUrl() {
    ...
    document.getElementById('confirmUrlImport').addEventListener('click', async () => {
        ...
        try {
            ...
            if (config.name !== 'NovaTV-Settings') throw '配置文件格式不正確';

            // 验证哈希
            const dataHash = await sha256(JSON.stringify(config.data));
            if (dataHash !== config.hash) throw '配置文件不匹配';
            ...
            showToast('配置文件導入成功，3 秒後自動更新本頁面。', 'success');
            setTimeout(() => {
...
async function importConfig() {
    showImportBox(async (file) => {
        try {
            ...
            showToast(`配置文件讀取出錯錯 (${message})`, 'error');
        }
    });
}
...
async function exportConfig() {
    ...
    config['name'] = 'NovaTV-Settings';  // 配置文件名，用于校验
    ...
    // 将配置数据保存为 JSON 文件
    saveStringAsFile(JSON.stringify(config), 'NovaTV-Settings_' + times + '.json');
}
