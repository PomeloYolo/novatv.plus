...
async function loadDoubanCoverFallback(imgElement, originalCoverUrl) {
    if (!originalCoverUrl || originalCoverUrl === '/image/movie.png' || originalCoverUrl === 'image/movie.png' || originalCoverUrl === 'null' || originalCoverUrl === 'undefined' || !originalCoverUrl.startsWith('http')) {
        imgElement.src = '/image/movie.png';
    ...
    try {
        ...
        imgElement.onerror = function() {
            ...
            if (typeof handlePosterError === 'function') {
                handlePosterError(this, imgElement.alt);
            } else {
                this.src = '/image/movie.png';
        ...
        if (typeof handlePosterError === 'function') {
            handlePosterError(imgElement, imgElement.alt);
        } else {
            imgElement.src = '/image/movie.png';
            imgElement.classList.add('object-contain');
        }
...
function getTmdbApiKey() {
    return "d1145e6ffe68b7e815c9f39ffdc7fd5c";
}

async function fetchTmdbPosterPath(title, year, type) {
    const apiKey = getTmdbApiKey();
    if (!apiKey || !title) return null;
    
    const searchType = type === "tv" ? "tv" : "movie";
    const baseUrl = `https://api.themoviedb.org/3/search/${searchType}`;
    ...
    try {
        ...
        console.error("獲取 TMDB 海報失敗:", e);
        return null;
    }
}

async function enhanceCoverWithTmdb(imgElement, item) {
    try {
        const title = item.title;
        const year = item.year;
        const type = doubanMovieTvCurrentSwitch;
        const posterPath = await fetchTmdbPosterPath(title, year, type);
        if (posterPath) {
            const tmdbUrl = `https://image.tmdb.org/t/p/w500${posterPath}`;
            // 保存當前的 src，如果設置失敗則可以回退
            const currentSrc = imgElement.src;
            imgElement.onerror = function() {
                this.onerror = null;
                // 如果 TMDB 失敗，嘗試使用通用海報備援函數
                if (typeof handlePosterError === 'function') {
                    handlePosterError(this, item.title);
                } else if (currentSrc && !currentSrc.includes('movie.png')) {
                    loadDoubanCoverFallback(this, item.cover);
                } else {
                    this.src = '/image/movie.png';
                    this.classList.add('object-contain');
                }
            };
            imgElement.src = tmdbUrl;
        }
    } catch (e) {
        console.error("更新 TMDB 海報失敗:", e);
    }
}
...
function renderDoubanCards(data, container) {
    ...
    if (!data.subjects || data.subjects.length === 0) {
        ...
        data.subjects.forEach(item => {
            ...
            // 處理封面 URL，防止 null/undefined 字串
            let coverUrl = item.cover || '';
            if (!coverUrl || coverUrl === 'null' || coverUrl === 'undefined' || !coverUrl.startsWith('http')) {
                coverUrl = '/image/movie.png';
            }
            
            const originalCoverUrl = coverUrl.replace(/'/g, "\\'");
            
            // 為不同設備優化 card.innerHTML 中的 img 部分
            card.innerHTML = `
                <div class="relative w-full aspect-[2/3] overflow-hidden cursor-pointer poster-container" onclick="fillAndSearchWithDouban('${safeTitle}')">
                    <img src="${coverUrl}" alt="${safeTitle}" 
                        class="w-full h-full ${coverUrl.includes('movie.png') ? 'object-contain' : 'object-cover'} transition-transform duration-500 hover:scale-110"
            ...
            if (imgEl) {
                enhanceCoverWithTmdb(imgEl, item);
            }
