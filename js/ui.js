// 處理圖片載入失敗的備援邏輯
async function handlePosterError(imgElement, title) {
    ...
    imgElement.onerror = null; // 暫時移除以防止異步操作期間觸發

    // 獲取電影標題，優先使用傳入的參數，其次是 alt 屬性
    const movieTitle = title || imgElement.alt || '';
    
    if (movieTitle && movieTitle !== 'undefined' && movieTitle !== 'null') {
        try {
            // 使用非凡資源 API 進行搜索
            const ffzyApi = `http://ffzy5.tv/api.php/provide/vod?ac=detail&wd=${encodeURIComponent(movieTitle)}`;
            const proxiedUrl = PROXY_URL + encodeURIComponent(ffzyApi);
            
            const response = await fetch(proxiedUrl);
            if (response.ok) {
                const data = await response.json();
                if (data.list && data.list.length > 0) {
                    const newPosterUrl = data.list[0].vod_pic;
                    if (newPosterUrl && newPosterUrl.startsWith('http')) {
                        // 如果是 HTTP 圖片，在 HTTPS 環境下可能需要代理
                        let finalUrl = newPosterUrl;
                        if (newPosterUrl.startsWith('http:')) {
                            finalUrl = PROXY_URL + encodeURIComponent(newPosterUrl);
                        }
            ...
            console.error('FFZY API 備援海報獲取失敗:', e);
        }
    }
            ...
            <div class="history-item favorite-item mb-3" data-id="${item.id}">
                <div class="flex items-center">
                    <div class="history-thumbnail mr-3">
                        ${(item.thumbnail && item.thumbnail.startsWith('http') && item.thumbnail !== 'null' && item.thumbnail !== 'undefined') ? 
                            `<img src="${item.thumbnail}" alt="${item.title}" class="w-16 h-9 object-cover rounded" onerror="handlePosterError(this, '${item.title.replace(/'/g, "\\'")}')" referrerpolicy="no-referrer">` : 
                            '<div class="w-16 h-9 bg-gray-700 flex items-center justify-center rounded"><img src="/image/movie.png" class="w-full h-full object-contain"></div>'}
                    </div>
